...
test.describe('Featured Work Page', () => {
  ...
  test('courses section displays correctly', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Courses & Educational Content' })).toBeVisible();
    ...
    await expect(page.getByText('Engineering Management Processes & Tools Fundamentals')).toBeVisible();
  });
  ...
  test('speaking availability section is present', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Interested in Having Me Speak?' })).toBeVisible();
  ...
  test('external links open in new tabs', async ({ page }) => {
    ...
    const externalLinks = page.locator('a[href*="pluralsight.com"], a[href*="youtube.com"], a[href*="youtu.be"]');
